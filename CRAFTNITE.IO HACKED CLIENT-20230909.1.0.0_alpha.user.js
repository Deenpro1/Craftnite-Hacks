// ==UserScript==
// @name         CRAFTNITE.IO HACKED CLIENT
// @namespace    http://tampermonkey.net/
// @version      20230909.1.0.0_alpha1
// @description  craftnite hackers toolkit
// @author       Deenpro1
// @match        https://craftnite.io/*
// @icon         https://www.google.com/s2/favicons?sz=64&domain=craftnite.io
// @grant        none
// @license      by-nd 4.0
// @downloadURL https://github.com/Deenpro1/Craftnite-Hacks/blob/main/CRAFTNITE.IO%20HACKED%20CLIENT-20230909.1.0.0_alpha1.user.js
// @updateURL https://github.com/Deenpro1/Craftnite-Hacks/blob/main/CRAFTNITE.IO%20HACKED%20CLIENT-20230909.1.0.0_alpha1.user.js
// ==/UserScript==
...
client.MAIN = function() {
    ...
    */
    client.hackList.innerHTML = "Deenpro1hack version "+client.version+"<br>";
    client.keyBindElement.innerHTML = "Keybinds:<br>z - open menu<br><br>";
    for(let i = 0; i < client.Hacks.length; i++){
        if(client.Hacks[i].isEnabled){
...
client.init = function() {
    ...
    var NoClip = new client.Hack(function (this_) {
        this_.originalCollision = GAME.a865.player.checkCollision;
        if(GAME.a865.player.checkCollision) {
            GAME.a865.player.checkCollision = function() { return false; };
        }
    }, function () {
        // Loop Logic
    }, function (this_) {
        if(this_.originalCollision) {
            GAME.a865.player.checkCollision = this_.originalCollision;
        }
    }, "NoClip", "Go through walls", "n");
    var KillAura = new client.Hack(function () {
    }, function () {
        G.othera822ers.forEach(function(player) {
            if(player && player.a240 && player.position) {
                var distance = GAME.a865.player.position.distanceTo(player.position);
                if(distance < Number(this.config.radius)) {
        ...
        this.type = this.config.radius + "m";
    }, function () {
    }, "KillAura", "kill everyióne in radius", "no keybind", false, {radius: {defaultValue: 10, type: 2}});

    var JumpBoost = new client.Hack(function (this_) {
        this_.originalJump = GAME.a865.player.jumpHeight || 2;
    }, function () {
        if(GAME.a865.player.jumpHeight !== undefined) {
            GAME.a865.player.jumpHeight = Number(this.config.height);
            this.type = this.config.height+"m";
        }
    }, function (this_) {
        if(GAME.a865.player.jumpHeight !== undefined) {
            GAME.a865.player.jumpHeight = this_.originalJump;
        }
    }, "JumpBoost", "Jump higher", "j", false, {height: {defaultValue: 5, type: 2}});

    var Teleport = new client.Hack(function () {
        // Init
    }, function () {
        if(this.config.activate && GAME.a865.player.position) {
            GAME.a865.player.position.x = Number(this.config.x);
            GAME.a865.player.position.y = Number(this.config.y);
            GAME.a865.player.position.z = Number(this.config.z);
            this.config.activate = false;
        }
    }, function () {
        // Cleanup
    }, "Teleport", "Teleport to coordinates", "no keybind", false, {
        x: {defaultValue: 0, type: 2},
        y: {defaultValue: 100, type: 2},
        z: {defaultValue: 0, type: 2},
        activate: {defaultValue: false, type: 0}
    });

    var FullBright = new client.Hack(function (this_) {
        if(GAME.a865.scene.fog) {
            this_.originalFogDensity = GAME.a865.scene.fog.density;
            this_.originalFogFar = GAME.a865.scene.fog.far;
            GAME.a865.scene.fog.density = 0;
        ...
        if(GAME.a865.scene.fog) {
            GAME.a865.scene.fog.density = this_.originalFogDensity;
            GAME.a865.scene.fog.far = this_.originalFogFar;
        }
    }, "FullBright", "Max Lighting", "b");

    var PlayerList = new client.Hack(function (this_) {
        this_.HUD = document.createElement("div");
        this_.HUD.style = "position: fixed; top: 10vh; left: 2vw; background-color: black; opacity: 0.8; padding: 10px; color: white; font-size: 14px; z-index: 1000; max-height: 40vh; overflow-y: auto;";
        document.body.appendChild(this_.HUD);
    }, function () {
        var playerCount = 0;
        var html = "<b style='font-size: 16px;'>🎮 Players Online:</b><br><br>";
        G.othera822ers.forEach(function(player, index) {
            if(player && player.a240 && GAME.a865.player.position) {
                playerCount++;
                var distance = Math.round(GAME.a865.player.position.distanceTo(player.position));
                html += "ID: " + index + " - 📍 " + distance + "m<br>";
            }
        });
        this.HUD.innerHTML = html + "<br><b>Total: " + playerCount + "</b>";
    }, function (this_) {
        if(this_.HUD) {
            this_.HUD.remove();
        }
    }, "PlayerList", "Show all players", "l");

    var TimeChanger = new client.Hack(function () {
        // Init
    }, function () {
        if(GAME.sun && GAME.sun.position) {
            var time = Number(this.config.time);
            GAME.sun.position.y = time;
            this.type = "Y:"+time;
        }
    }, function () {
        // Reset
    }, "TimeChanger", "Change time {client side]", "no keybind", false, {time: {defaultValue: 100, type: 2}});

    var FOVChanger = new client.Hack(function (this_) {
        if(GAME.a865.player.camera) {
            this_.originalFOV = GAME.a865.player.camera.fov;
        }
    }, function () {
        if(GAME.a865.player.camera) {
            GAME.a865.player.camera.fov = Number(this.config.fov);
            GAME.a865.player.camera.updateProjectionMatrix();
            this.type = this.config.fov+"°";
        }
    }, function (this_) {
        if(GAME.a865.player.camera && this_.originalFOV) {
            GAME.a865.player.camera.fov = this_.originalFOV;
            GAME.a865.player.camera.updateProjectionMatrix();
        }
    }, "FOVChanger", "Change view", "no keybind", false, {fov: {defaultValue: 90, type: 2}});
    var AutoRespawn = new client.Hack(function () {
        // Init
    }, function () {
        if(GAME.respawnButton && GAME.respawnButton.style.display !== "none") {
            GAME.respawnButton.click();
        }
    }, function () {
        // Cleanup
    }, "AutoRespawn", "Automatically respawn", "no keybind");

    var KillStats = new client.Hack(function (this_) {
        this_.HUD = document.createElement("div");
        this_.HUD.style = "position: fixed; bottom: 10vh; right: 2vw; background-color: rgba(0,0,0,0.7); padding: 15px; color: #fff; font-size: 18px; z-index: 1000; border: 2px solid lime; border-radius: 10px;";
        document.body.appendChild(this_.HUD);
        this_.totalKills = 0;
        this_.deaths = 0;
        this_.lastKills = 0;
    }, function () {
        if(GAME.a865.player.a649 !== undefined) {
            if(GAME.a865.player.a649 < this.lastKills) {
                this.deaths++;
            }
            if(GAME.a865.player.a649 > this.lastKills) {
                this.totalKills += (GAME.a865.player.a649 - this.lastKills);
            }
            this.lastKills = GAME.a865.player.a649;
            var kd = this.deaths > 0 ? (this.totalKills / this.deaths).toFixed(2) : this.totalKills;
            this.HUD.innerHTML = "⚔️ <b>Kills:</b> " + this.totalKills + "<br>💀 <b>Deaths:</b> " + this.deaths + "<br>📊 <b>K/D:</b> " + kd;
        }
    }, function (this_) {
        if(this_.HUD) {
            this_.HUD.remove();
        }
    }, "KillStats", "Show Kill/Death Statistics", "no keybind");
    ...
    //menu elements
    new client.MenuElement([Fly, WaterLevel, SpeedHack, NoClip, JumpBoost, Teleport], "Movement", "0%", "0%");
    new client.MenuElement([RapidFire, InfAmmo, KillAura, TriggerBot], "Combat", "25%", "0%");
    new client.MenuElement([ESP, InfoHUD, NoFog, FullBright, PlayerList, TimeChanger, FOVChanger], "Render", "50%", "0%");
    new client.MenuElement([ChatSpam, AutoRespawn, KillStats, stophacks], "Game", "75%", "0%");
    //win message
